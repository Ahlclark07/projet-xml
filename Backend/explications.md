# API REST Cinemas (Node.js stdlib + SQLite builtin)

## Lancer le serveur
- Prerequis : Node.js >= 22 (utilise `node:sqlite` experimental, aucune dependance externe).
- Commande : `node src/server.js`
- Le serveur ecoute sur `http://0.0.0.0:8000` et cree `data.db` a la racine.

## Authentification
- Les routes protegees necessitent l'entete `X-API-Key`.
- Creation d'un compte exploitant :  
  `POST /owners`  
  Corps JSON : `{"name": "Exploitant", "username": "user1", "password": "secret"}`  
  Reponse 201 : inclut `api_key`.
...
## Notes techniques
- Stockage : SQLite `data.db`, tables `owners`, `cinemas`, `films`, `seances`.
- Dates attendues : `YYYY-MM-DD`.
- `image_url` est optionnel (texte) pour stocker un lien d'affiche.
- Module `node:sqlite` est experimental dans Node 22 -> warning au lancement, normal.

## Jeu de donnees de demo
- Script : `node src/seed.js`
- Ce script :
  - Cree un owner admin (`username`: admin, `password`: admin, `api_key`: `admin`),
  - Cree un cinema par defaut,
  - Insere jusqu'a 10 films avec affiches (URLs publiques) et genere 3 seances aleatoires par film.
